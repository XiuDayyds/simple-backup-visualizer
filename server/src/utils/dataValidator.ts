...
export function validateDiaryData(data: any): ValidationResult {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // content字段是可选的，但如果没有content，必须有album、audio或music
      if (!entry.content && !entry.album && !entry.audio && !entry.music) {
        errors.push(`第${i + 1}条记录必须至少包含content、album、audio或music中的一个`);
        continue;
      }
      ...
      if (entry.album) {
        if (!Array.isArray(entry.album)) {
          ...
          // 统计图片和视频数量
          const hasImages = entry.album.some((item: any) => item.type === 'image' || item.type === 'live_photo');
          ...
          entry.album.forEach((item: any, idx: number) => {
            if (!item.type) {
              errors.push(`第${i + 1}条记录的album[${idx}]缺少type字段`);
            } else if (!item.url || item.url === '') {
              // 空URL的媒体项会被忽略，不作为错误
              console.warn(`第${i + 1}条记录的album[${idx}]的URL为空，将被忽略`);
            } else if (!['image', 'video', 'live_photo'].includes(item.type)) {
              errors.push(`第${i + 1}条记录的album[${idx}]的type字段必须是'image'、'video'或'live_photo'`);
            }
          });
        }
      }
      ...
      if (entry.music) {
        if (typeof entry.music !== 'string') {
          errors.push(`第${i + 1}条记录的music字段必须是字符串`);
        } else {
          statistics.withMusic++;
        }
      }
