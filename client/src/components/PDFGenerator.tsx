...
import { useSmoothProgress } from '../utils/progressSmooth';
...
export default function PDFGenerator({ data, onBack, onReset }: PDFGeneratorProps) {
  ...
  const { displayProgress, setProgress, complete, reset } = useSmoothProgress();
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const result = await generatePDF(file, options, (progressData) => {
        ...
        setProgressMessage(progressData.message || '正在处理...');
      });

      if (result.success && result.downloadUrl) {
        complete();
        setDownloadUrl(result.downloadUrl);
        setProgressMessage('PDF生成完成！');
            ...
            {isGenerating && (
              <div className="space-y-3">
                ...
                <p className="text-sm text-gray-500 text-center">
                  {progressMessage || '正在处理图片和生成PDF，请稍候...'}
                </p>
              </div>
            )}
