...
export function processDiaryEntry(entry: DiaryEntry): ProcessedDiaryEntry {
  ...
  const formattedDate = formatDate(safeEntry.date);
  const yearMonth = getYearMonth(safeEntry.date);
  const timestamp = new Date(safeEntry.date).getTime();
...
export function processDiaryData(entries: DiaryEntry[]): ProcessedDiaryEntry[] {
  return entries
    ...
    .filter(entry => !isNaN(entry.timestamp)) // 过滤无效时间戳
...
export function validateDiaryData(data: any): string | null {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // 允许在有album、audio或music的情况下缺少content字段
    if (!entry.content && !entry.album && !entry.audio && !entry.music) {
      return `第${i + 1}条记录缺少内容：需要至少包含content、album、audio或music字段之一`;
    }
...
export function getDataStatistics(entries: ProcessedDiaryEntry[]) {
  ...
  const withImages = entries.filter(e => e.album && Array.isArray(e.album) && e.album.some(item => item && item.type === 'image')).length;
  const withVideos = entries.filter(e => e.album && Array.isArray(e.album) && e.album.some(item => item && item.type === 'video')).length;
  const withAudio = entries.filter(e => e.audio).length;
  const withMusic = entries.filter(e => e.music).length;
  const withTags = entries.filter(e => e.tags && Array.isArray(e.tags) && e.tags.length > 0).length;
