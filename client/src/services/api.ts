...
export async function generateDocument(
  ...
  // Generate a jobId on client side first
  const jobId = `job-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  formData.append('jobId', jobId);

  // Start tracking progress BEFORE sending the request
  let tracker: any = null;
  if (onProgress) {
    tracker = trackProgress(jobId, (progressData) => {
      // Convert SSE progress to UploadProgress format
      const progress: UploadProgress = {
        loaded: progressData.progress,
        total: 100,
  ...
  const response = await api.post(endpoint, formData, {
    ...
    onUploadProgress: (progressEvent: AxiosProgressEvent) => {
      if (onProgress && progressEvent.total) {
        // Only report upload progress up to 10%
        const uploadPercentage = Math.round((progressEvent.loaded * 100) / progressEvent.total);
        const progress: UploadProgress = {
          loaded: progressEvent.loaded,
          total: progressEvent.total,
          percentage: Math.min(uploadPercentage * 0.1, 10), // Cap at 10%
          message: '上传文件中...',
        };
        onProgress(progress);
      }
  ...
  // Clean up tracker after a timeout (in case completion event is missed)
  if (tracker) {
    setTimeout(() => tracker.disconnect(), 60000); // 1 minute timeout
  }
